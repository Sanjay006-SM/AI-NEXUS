...
export default function AnalyticsDashboard() {
  ...
  const riskOverTimeData = analytics?.riskOverTimeData || [];
  const providerData = analytics?.providerData || [];
  const totalIdentities = providerData.reduce((acc: any, curr: any) => acc + curr.value, 0);

  if (isLoading) {
    return <div className="p-4 text-text-muted">Loading analytics...</div>;
  }
